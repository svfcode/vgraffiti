...
export function scaledStrokeSize(stroke: StoredStroke, map: MapContext): number {
  if (
    stroke.coordSpace === "streetview" ||
    stroke.coordSpace === "screen" ||
    stroke.coordSpace === "viewmemory"
  ) {
...
export function projectStoredStroke(
  ...
  if (stroke.coordSpace === "screen") {
    throw new Error("projectStoredStroke: screen strokes render without projection");
  }
