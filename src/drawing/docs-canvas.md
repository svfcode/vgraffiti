...
**Точный (дробный) zoom.** Web Mercator завязан на масштаб `256 · 2^zoom`. Карты зумируют дробно (12.3, 9.7), поэтому zoom **не округляется** ни в мосте, ни в `map-context` — иначе штрих «съезжал» бы и масштабировался неверно при изменении масштаба.

**Источник live-данных карты — MAIN-world мост.** `entrypoints/map-bridge.content.ts` (`world: "MAIN"`, `document_start`) запускает `runMapBridge()` (`src/lib/map-bridge-main.ts`). Он работает в контексте страницы (доступ к `ymaps`) и **не блокируется CSP** Яндекс/Google (в отличие от старой inline-`<script>` инъекции). Мост читает живые `getCenter()`/`getZoom()` и шлёт их в изолированный мир через `postMessage` (протокол — `map-bridge-protocol.ts`), коалесцируя отправку до 1 раза в кадр. Изолированный `map-live-probe.ts` принимает их и зовёт listener → `host.mapContext` → `scheduleRedraw`. На случай, если мост недоступен, в `installMapBinding` есть fallback — опрос URL (`ll=`/`z=`) раз в 250 мс.

### Нативный слой ymaps (вариант A, без дрифта)

Репроекция geo→экран каждый кадр (вариант C) принципиально отстаёт от анимации карты (GPU двигает тайлы, а `getZoom()` во время зума отдаёт целевое значение) → рисунок «плывёт». Чтобы этого не было, **завершённые штрихи рисует сам ymaps** как гео-объекты:

- При изменении `strokes` (finish / undo / redo / clear) overlay шлёт их мосту: `syncStrokesToBridge()` → `broadcastStrokes()` → `STROKES_MSG`.
- Мост (`runMapBridge`) строит `ymaps.GeoObject` (`LineString` для кисти/стрелки, `Rectangle` для квадрата) в собственной `GeoObjectCollection` на карте. ymaps двигает/масштабирует их на GPU синхронно с тайлами → **дрифта нет**.
- Когда мост нашёл карту, он шлёт `RENDER_MODE_MSG {native:true}`. Overlay ставит `host.mapNativeRender = true`, и `redraw()` перестаёт рисовать завершённые штрихи на canvas — на нём остаётся только **текущий жест** (его рисуем в screen-space, карта в режиме «Рис» не двигается).
- Если ymaps не найден (другой сайт, сбой) — `native` не приходит, overlay рисует всё сам (вариант C с URL-fallback).

Ограничения нативного слоя: **ластик** не имеет смысла как вектор и в нативный слой не уходит; **кисть** становится линией постоянной толщины (без pressure-стиля `perfect-freehand`); у **стрелки** нет наконечника (рисуется как линия). Геометрия (форма, привязка к месту) при этом точная на любом масштабе.
